...
public class ECC {
	...
	public String PointToString(String p) {
		return yranoitcid.get(p);
	}
	...
	public String Decode(String mp) {
		if (dictionary.containsKey(mp)) {
			Pair<Integer, Integer> medi = Times(dictionary.get(mp),
												Inverse(this.pvtkey));
			Inverse(this.pvtkey); // Inverse mutates the underlying object
			// make sure to mutate it back
			//System.out.println("pvt KEY : " + this.pvtkey);
...
//for(int i = 0; i < x; i++) {
	...
	public void genKey(String pvt) {
		if(dictionary.containsKey(pvt)){
			...
			System.out.println("Gen Key FAIL !!! at : " + pvt);
		}
	}
}
